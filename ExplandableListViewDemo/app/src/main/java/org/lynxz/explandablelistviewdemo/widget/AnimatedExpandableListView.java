...
public class AnimatedExpandableListView extends ExpandableListView {
    ...
    public boolean expandGroupWithAnimation(int groupPos) {
        ...
        /*
        * expandGroup(groupPos, true)的动画是在groupItem可以显示,并按需放置于页面顶端,同时保证childList尽量显示完全
        * 因此动画形式类似于从底部显示childList,若页面childList可以在一屏内显示完全,则不会有动画
        * 因此这里限定最有一个group才使用系统提供的动画
        * */
        if (lastGroup && Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            return expandGroup(groupPos, true);
        }

        //获取点击的item在整个listView(包含group和已显示的childItem组合起来的listView)中的实际位置
        ...
        //expandGroup(int)会去调用 getChildView(),并在该方法中进行动画,动画结束后notifyDataSetChanged
        //并显示真实的childListView
        return expandGroup(groupPos);
    }
